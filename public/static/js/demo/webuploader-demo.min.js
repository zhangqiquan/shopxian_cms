jQuery(function() {
	...
	n.on( 'uploadSuccess', function( file,data ) {
		...
		//try{
			var all_NoNull=true;
			var parent_all_li=parent.document.getElementById("tutu").getElementsByTagName("li");
			for(var i=0;i<parent_all_li.length;i++){
				if(parent_all_li[i].getElementsByTagName("input")[0].value==''){
					all_NoNull=false;
					parent_all_li[i].getElementsByTagName("input")[0].value=data.path;
					parent_all_li[i].getElementsByTagName("img")[0].src='http://'+window.location.host+'/'+data.path.replace('/level0/','/level5/');
					break;
				}
			}
			console.log(parent_all_li);
		//}catch(e){
		//}
    	console.log(data);
	}),
